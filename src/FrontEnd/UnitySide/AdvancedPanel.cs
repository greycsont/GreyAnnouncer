...
public class AdvancedPanel : MonoBehaviour
{
    public void Awake()
    {
        ...
        var lpToggle = UIBuilder.AddToggle(lpRow, preferredWidth: 30);
        lpToggle.isOn = Setting.isLowPassFilterEnabled;
        lpToggle.onValueChanged.AddListener(v => Setting.isLowPassFilterEnabled = v);
        ...
        var ffToggle = UIBuilder.AddToggle(ffRow, preferredWidth: 30);
        ffToggle.isOn = Setting.isFFmpegSupportEnabled;
        ffToggle.onValueChanged.AddListener(v => Setting.isFFmpegSupportEnabled = v);
